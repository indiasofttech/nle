...
<!-- <tr>
  ...
  <td><?phP echo 'Select Template';?></td>
  <td>
  <select name="spage" required > 
  <option value="" >Select Template</option>
  <?php $req = Sql_Query(sprintf('select * from phplist_subscribepage  where  active=1 order by title')); print_r($req);
  while ($p = Sql_Fetch_Array($req)) {  ?><option value="<?=$p[id]?>" > <?=$p[title]?></option>
<?php }?>
   </select>
  </td>
</tr> -->
